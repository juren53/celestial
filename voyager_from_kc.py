...
def main():
    ...
    for name, sc_id in spacecraft.items():
        ...
        try:
            ...
            # Request additional quantities including heliocentric velocity
            eph = obj.ephemerides(quantities='1,9,20,23')
            ...
            current_velocity_kms = eph['delta_rate'][0]  # Velocity in km/s relative to Earth
            moving_direction = "away from" if current_velocity_kms > 0 else "toward"
            
            # Get heliocentric velocity (relative to the Sun)
            # The 'v' field represents the heliocentric velocity in km/s
            helio_velocity_kms = eph['v'][0]  # Heliocentric velocity in km/s
            helio_velocity_kph = helio_velocity_kms * 3600  # Convert to km/h
            helio_velocity_mph = helio_velocity_kph * 0.621371  # Convert to mph
            ...
            print(f"    Heliocentric velocity: {helio_velocity_kms:.2f} km/s ({helio_velocity_kph:.2f} kph, {helio_velocity_mph:.2f} mph)")
